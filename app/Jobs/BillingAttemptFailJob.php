...
class BillingAttemptFailJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        if ($shop === null) {
            Log::warning('Billing attempt failure webhook for unknown shop', [
        ...
        if (! $contractGid) {
            Log::warning('Billing attempt failure webhook missing contract id', [
                'shop_domain' => $shopDomain,
                'payload' => $payload,
            ]);
        ...
        try {
            ...
            if (! $subscription) {
                Log::warning('Billing attempt failure could not resolve subscription', [
                    'shop_id' => $shop->id,
            ...
            $cycleIndex = $payload['cycle_index'] ?? $this->guessCycleIndex($subscription);

            $recoveryService->handleFailure(
                $shop,
                $subscription,
                (int) $cycleIndex,
                $payload['error_code'],
            ...
            Log::error('BillingAttemptFailJob failed', [
                'shop_id' => $shop->id,
    ...
    private function guessCycleIndex(Subscription $subscription): int
    {
        $open = $subscription->paymentRecoveries()
            ->whereIn('status', [
                \App\Models\SubscriptionPaymentRecovery::STATUS_AWAITING_RETRY,
                \App\Models\SubscriptionPaymentRecovery::STATUS_PROCESSING,
            ])
            ->latest('id')
            ->first();

        if ($open) {
            return (int) $open->cycle_index;
        }
